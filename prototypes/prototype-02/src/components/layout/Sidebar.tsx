import { Home, Bell, User, Settings, LogOut, ShieldCheck } from "lucide-react";
...
export function Sidebar() {
  ...
  const mlStatus = useFilterStore((s) => s.mlStatus);
      ...
      {filterEnabled && (
        <div className="mb-3 px-3 py-2 rounded-lg bg-muted/50 text-xs text-muted-foreground">
          <div className="flex items-center gap-1.5 mb-1">
            ...
            <span className="font-medium text-foreground">フィルター</span>
            {mlStatus === "loading" && (
              <span className="ml-auto text-[10px] text-blue-500">AI 読込中...</span>
            )}
            {mlStatus === "ready" && (
              <span className="ml-auto text-[10px] text-green-500">AI 稼働中</span>
            )}
          </div>
          <p>
            今回のセッション:{" "}
            <span className="font-medium text-foreground">{filteredCount} 件</span> を保護
          </p>
        </div>
      )}
