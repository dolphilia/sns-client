...
| Monaco Editor | CSS 編集から開始し、後で JS 編集にも広げる。 |
...
Monaco Editor を使い、最初は CSS 編集だけを有効にする。
...
- `apps/sns-browser/` を作る
- Electron + React + Vite の最小起動
- `WebContentsView` で X / Threads / mixi2 のどれか 1 つを表示
- 戻る / 進む / 再読み込み
...
- 高度モードを追加
- `document-end` / `document-idle` の JS 注入
- MutationObserver ベースの再適用
- JS 編集前のセキュリティレビュー
...
- CSS ルールをオン/オフできる
- Monaco Editor で編集した CSS が再適用される
- アプリ再起動後も設定が残る
...
- `WebContentsView` をサイトごとに保持するか、1 つを使い回すか
- session partition を SNS ごとに分けるか、実験アプリ内で 1 つにまとめるか
- X / Threads / mixi2 のプリセットを CSS だけでどこまで実現できるか
- JS 高度モードをいつ解禁するか
